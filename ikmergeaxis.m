...
gripper_orientation = -pi/2;% orientation of gripper with respect to world frame
...
theta3 = -acos(arg);% two sols + and - when feasible? derived using cosine rule(I think)
...
theta2 = atan(theta2_num/theta2_denom); %derived by defining wrist joint using basic trig and theta2 and theta3, then rearranged

% theta4 
theta4 = gripper_orientation - theta3 - pi/2 + theta2;% gripper orientation with respect to world frame is sum of total rotations of links in z-x plane
...
theta2 =  - (theta2 - (deg2rad(90) - constant));
theta3 = theta3 + constant;
