...
% 
%         %put cube down
%         pointsList = [pointsList; [start_list(i,1), start_list(i,2), cube_grab_side_z, thetaG_horizontal, open_value]];
% 
...
%     
%         %put cube down
%         pointsList = [pointsList; [end_list(i,1), end_list(i,2), current_place_down_height, thetaG_down, open_value]];
%         
%         %move back to safe distance
%         pointsList = [pointsList; [end_list(i,1), end_list(i,2), safe_dist, thetaG_down, open_value]];
% 
...
% 
%     end
%     current_place_down_height = current_place_down_height + cube_grab_top_z;
%     i = i+1;
% end

%% no while loop %%
 
% % cube 1 Go to safe distance above cube 
...
%pick cube up
pointsList = [pointsList; [start_list(1,1), start_list(1,2), safe_dist, thetaG_down, close_value]];
%rotate cube
pointsList = [pointsList; [start_list(1,1), start_list(1,2), safe_dist, thetaG_horizontal, close_value]];
%put cube down
pointsList = [pointsList; [start_list(1,1), start_list(1,2), cube_grab_side_z, thetaG_horizontal, open_value]];
%move back to safe distance
pointsList = [pointsList; [end_list(1,1), end_list(1,2), safe_dist, thetaG_horizontal, open_value]];
% %rotate end effector (not needed)
% pointsList = [pointsList; [end_list(1,1), end_list(1,2), safe_dist, thetaG_down, open_value]];
%%increment height to place down bloack as blocks are stacked
current_place_down_height = current_place_down_height + cube_grab_top_z- 0.005; 


% % cube 2  Go to safe distance above cube 
...
%pick cube up
pointsList = [pointsList; [start_list(2,1), start_list(2,2), safe_dist, thetaG_down, close_value]];
%move to end point
pointsList = [pointsList; [end_list(2,1), end_list(2,2), current_place_down_height + 0.04, thetaG_down, close_value]];
%put cube down
pointsList = [pointsList; [end_list(2,1), end_list(2,2), current_place_down_height, thetaG_down, open_value]];
%move back to safe distance
pointsList = [pointsList; [end_list(2,1), end_list(2,2), current_place_down_height + 0.04, thetaG_down, open_value]];
%%increment height to place down bloack as blocks are stacked
current_place_down_height = current_place_down_height + cube_grab_top_z - 0.005; 


% cube 3 Go to safe distance above cube 
pointsList = [pointsList; [start_list(3,1), start_list(3,2), safe_dist, thetaG_down, open_value]];
% Grab cube
pointsList = [pointsList; [start_list(3,1), start_list(3,2), cube_grab_top_z, thetaG_down, close_value]];
%pick cube up
pointsList = [pointsList; [start_list(3,1), start_list(3,2), safe_dist, thetaG_down, close_value]];
%move to end point
pointsList = [pointsList; [end_list(3,1), end_list(3,2), current_place_down_height + 0.04, thetaG_horizontal, close_value]];
%put cube down
pointsList = [pointsList; [end_list(3,1), end_list(3,2), current_place_down_height, thetaG_horizontal, open_value]];
%move back to safe distance
pointsList = [pointsList; [end_list(3,1), end_list(3,2), current_place_down_height + 0.04, thetaG_horizontal, open_value]];
